...
> **工作 vs 檢視：** 詳見 [`OPS_MAP_WORK_BROWSE.md`](./OPS_MAP_WORK_BROWSE.md)。工作模式地圖僅用官方歸檔時間窗；檢視模式用 URL 查詢時間，且不影響歸檔範圍。

1. **畫分區**（工作模式）：右側「＋ 畫分區」→ 點地圖頂點 → 完成 → 儲存
2. **編輯**：點選紅框分區 → 浮卡「編輯邊界」→ 調整頂點 → 儲存
3. **回報**：彩色圓點；點選可審核／旗標；**檢視模式**左下可設查詢時間篩選
4. **危機歸檔**（工作模式）：選危機、設**官方**時間窗 → 預覽 → 執行歸檔
