...
> 最後更新：**2026-05-27**（第二階段離線地圖 UX 修復已 push；明日接 C1 驗收與待辦清單見下）
...
| 佇列列表／失敗重試 UI | 待做 |
| PWA / SW（離線可開首頁） | **已做**（`vite-plugin-pwa` 產生 `sw.js` + precache + runtime cache） |
| 離線地圖瓦片預下載（**半徑 2 km**，方框約 **4×4 km**；z14–z17） | **已做**（`tilePrefetch.ts` + `CachedOsmTileLayer` + IndexedDB） |
| 下載預覽／已存區域方框 | **已做**（橘色預覽、藍色已存、`DownloadPreviewLayer`） |
| 離線選區飛至 z17、不鎖縮放 | **已做**（`offlineZoomLimits=null`；恢復連線自動 `reloadWindow`，無多餘橫幅） |
| 離線視野 maxBounds 鎖定 | **已移除**（改預下載 + 方框導覽） |
| PWA / SW | **已做**（`vite-plugin-pwa`） |

### C1 / C1.5 明日待辦（優先順序）

| # | 項目 | 說明 |
|---|------|------|
| 1 | **飛航模式 E2E** | 連線下載地圖 → 離線新增含圖回報 → 恢復連線 → Neon／佇列清空 |
| 2 | **實機驗收離線地圖** | 點已存區域應 z17 可放大；平移超出方框可接受局部灰屏 |
| 3 | **佇列明細 UI** | 待同步列表、失敗重試（banner 僅顯示筆數） |
| 4 | **同步狀態機** | 對齊 `MODULE_SPEC`（`uploading_image` 等，可選） |
| 5 | **Neon migration** | 生產執行 `005_unspecified_crisis.sql`（`demo-taipei` → `unspecified`） |
| 6 | **程式清理** | 刪除未用的 `useStableOnlineRestore.ts`；i18n 移除已廢棄的 `onlineRestored` 等 key |
| 7 | **選做** | 預下載 z18（更細但瓦片量更大）；Capacitor Network／背景同步 |

### C2 摘要（多數已落地）

- PWA 殼、可安裝、離線開首頁：**已完成**。
- 待加強：SW 更新策略、安裝提示 UX、與 C1 佇列 UI 一併驗收。

### 下載範圍說明（常見疑問）

- 程式常數 `DEFAULT_RADIUS_KM = 2` 指**圓形半徑**（中心 ±2 km）。
- 地圖上橘／藍方框是圓的**外接正方形**，邊長約 **4 km**（緯度方向約 4 km；經度方向隨緯度略異）。
- UI 已改為「半徑 2 km，方框約 4×4 km」以免與視覺不符。
...
| ② | 離線 PWA | **進行中**（C1 填報 + C1.5 瓦片預下載；見下方待辦） |

| ③ | 管理員畫框 / RBAC | **3a–3c 完成**（map-first `/ops/map`、多邊形編輯、危機歸檔、稽核；見 `CLASSIFICATION_AND_ZONES.md`） |
...
| 2026-06-08 | Phase 3b/3c：migration 007、歸檔預覽/執行、稽核 log；營運後台改 map-first `/ops/map`、多邊形編輯 |
| 2026-06-08 | Phase 3a：migration 006、`/v1/ops` 登入與分區 API、前端 `/ops/login` `/ops/zones`、儀表板分區篩選 |
| 2026-05-27 | 離線：移除恢復連線橫幅、自動 reload；離線已存區 z17 飛行且不鎖縮放；下載標題標明半徑 vs 4×4 方框 |
